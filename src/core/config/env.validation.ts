...
export const envValidationSchema = Joi.object({
  ...
  AI_SERVICE_URL: Joi.string().uri().default('https://nondesigned-alexis-unpliantly.ngrok-free.dev'),
  AI_SERVICE_API_KEY: Joi.string().default('dev-ai-key-change-me'),
  AI_SERVICE_TIMEOUT_MS: Joi.number().integer().min(1000).max(120000).default(5000),
  AI_SERVICE_USE_STUB: Joi.string().valid('true', 'false').default('true'),
  AI_CHAT_URL: Joi.string().uri().optional(),
  AI_CHAT_TIMEOUT_MS: Joi.number().integer().min(1000).max(120000).optional(),
  AI_CHAT_USE_STUB: Joi.string().valid('true', 'false').default('false'),
});
